...
from e2e.helpers.waf_policies_helper import (
    name_regex,
    restrict,
    waf_test_config,
    ignore_alphanum,
...
class TestWAFPolicies:
    def test_section_overlong_length_with_default_value(self, target, section):
        ...
        ), f"Response not 403 despite overlong {section}"
    ...
    def test_section_short_length_with_default_value(self, target, section):
        ...
        ), f"Response not 200 despite correct length {section}"
    ...
    @pytest.mark.parametrize(
        "uri, sect, block_name, pass_name",
        [
            ("abcd", "cookies", "sasa", "block_cookie"),
            ("bcde", "headers", "erer", "block_header"),
            ("cdef", "params", "bnbnb", "block_arg"),
        ],
    )
    def test_block_cookie_header_arg_by_name(
        self, target, uri, sect, block_name, pass_name
    ):
        response = target.query(f"/{uri}", **{sect: {"foobar": block_name}})
        assert (
            response.status_code == 403
        ), f"Response not 403 despite {sect} does not match pattern"
        response = target.query(f"/{uri}", **{sect: {"foobar": pass_name}})
        assert (
            response.status_code == 200
        ), f"Response not 200 despite {sect} matches pattern"

    @pytest.mark.parametrize(
        "uri, sect, block_name",
        [
            ("ig_alpha_cook", "cookies", "sasa"),
    ...
    def test_ignore_alphanumeric(self, target, uri, sect, block_name):
        ...
        assert (
            response.status_code == 200
        ), "Response not 200 despite ignore alphanumeric is True"

    def test_allowlisted_value(self, section, name_regex, restrict, target):
